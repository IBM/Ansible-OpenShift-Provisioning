...
**disconnected_enabled** | Enable disconnected/air-gapped installation mode. Set to `true` for environments without direct internet access. When enabled, the playbooks will use a local container registry or existing registry for mirroring OCP images and operators. Default value is `false`. See [disconnected installation guide](run-the-playbooks-for-disconnected-install.md) for complete setup instructions. | false
